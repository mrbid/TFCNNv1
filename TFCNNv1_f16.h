...
float unpackFloat(const f16 x) { // IEEE-754 16-bit floating-point format (without infinity): 1-5-10, exp-15, +-131008.0, +-6.1035156E-5, +-5.9604645E-8, 3.311 digits
    const uint e = (x&0x7C00)>>10; // exponent
    const uint m = (x&0x03FF)<<13; // mantissa
    const uint v = as_uint((float)m)>>23; // evil log2 bit hack to count leading zeros in denormalized format
    return as_float((x&0x8000)<<16 | (e!=0)*((e+112)<<23|m) | ((e==0)&(m!=0))*((v-37)<<23|((m<<(150-v))&0x007FE000))); // sign : normalized : denormalized
}
f16 packFloat(const float x) { // IEEE-754 16-bit floating-point format (without infinity): 1-5-10, exp-15, +-131008.0, +-6.1035156E-5, +-5.9604645E-8, 3.311 digits
...
static inline float Momentum(network* net, const float input, const float error, f16* momentum)
{
...
static inline float Nesterov(network* net, const float input, const float error, f16* momentum)
{
...
static inline float ADAGrad(network* net, const float input, const float error, f16* momentum)
{
...
static inline float RMSProp(network* net, const float input, const float error, f16* momentum)
{
    const float err = error * input;
    const float momu = unpackFloat(momentum[0]);
...
static inline float Optimiser(network* net, const float input, const float error, f16* momentum)
{
    if(net->optimiser == 1)
...
{
    p->data = malloc(weights * sizeof(f16));
    if(p->data == NULL)
        return ERROR_ALLOC_PERCEPTRON_DATAWEIGHTS_FAIL;

    p->momentum = malloc(weights * sizeof(f16));
    if(p->momentum == NULL)
    {
        free(p->data);
...
int saveWeights(network* net, const char* file)
{
    ...
    if(f != NULL)
    {
        for(int i = 0; i < net->num_layers-1; i++)
        {
            for(int j = 0; j < net->num_layerunits; j++)
            {
                if(fwrite(&net->layer[i][j].data[0], 1, net->layer[i][j].weights*sizeof(f16), f) != net->layer[i][j].weights*sizeof(f16))
                {
                    fclose(f);
                    return -1;
                }
                
                if(fwrite(&net->layer[i][j].momentum[0], 1, net->layer[i][j].weights*sizeof(f16), f) != net->layer[i][j].weights*sizeof(f16))
                {
                    fclose(f);
                    return -1;
                }

                if(fwrite(&net->layer[i][j].bias, 1, sizeof(f16), f) != sizeof(f16))
                {
                    fclose(f);
                    return -1;
                }
                
                if(fwrite(&net->layer[i][j].bias_momentum, 1, sizeof(f16), f) != sizeof(f16))
                {
                    fclose(f);
                    return -1;
                }
        ...
        if(fwrite(&net->layer[net->num_layers-1][0].data[0], 1, net->layer[net->num_layers-1][0].weights*sizeof(f16), f) != net->layer[net->num_layers-1][0].weights*sizeof(f16))
        {
            fclose(f);
            return -1;
        }
        
        if(fwrite(&net->layer[net->num_layers-1][0].momentum[0], 1, net->layer[net->num_layers-1][0].weights*sizeof(f16), f) != net->layer[net->num_layers-1][0].weights*sizeof(f16))
        {
            fclose(f);
            return -1;
        }

        if(fwrite(&net->layer[net->num_layers-1][0].bias, 1, sizeof(f16), f) != sizeof(f16))
        {
            fclose(f);
            return -1;
        }
        
        if(fwrite(&net->layer[net->num_layers-1][0].bias_momentum, 1, sizeof(f16), f) != sizeof(f16))
        {
            fclose(f);
            return -1;
        }
...
int loadWeights(network* net, const char* file)
{
    ...
    for(int i = 0; i < net->num_layers-1; i++)
    {
        for(int j = 0; j < net->num_layerunits; j++)
        {
            if(fread(&net->layer[i][j].data[0], 1, net->layer[i][j].weights*sizeof(f16), f) != net->layer[i][j].weights*sizeof(f16))
            {
                fclose(f);
                return -1;
            }

            if(fread(&net->layer[i][j].momentum[0], 1, net->layer[i][j].weights*sizeof(f16), f) != net->layer[i][j].weights*sizeof(f16))
            {
                fclose(f);
                return -1;
            }

            if(fread(&net->layer[i][j].bias, 1, sizeof(f16), f) != sizeof(f16))
            {
                fclose(f);
                return -1;
            }

            if(fread(&net->layer[i][j].bias_momentum, 1, sizeof(f16), f) != sizeof(f16))
            {
                fclose(f);
                return -1;
            }
    ...
    if(fread(&net->layer[net->num_layers-1][0].data[0], 1, net->layer[net->num_layers-1][0].weights*sizeof(f16), f) != net->layer[net->num_layers-1][0].weights*sizeof(f16))
    {
        fclose(f);
        return -1;
    }

    if(fread(&net->layer[net->num_layers-1][0].momentum[0], 1, net->layer[net->num_layers-1][0].weights*sizeof(f16), f) != net->layer[net->num_layers-1][0].weights*sizeof(f16))
    {
        fclose(f);
        return -1;
    }

    if(fread(&net->layer[net->num_layers-1][0].bias, 1, sizeof(f16), f) != sizeof(f16))
    {
        fclose(f);
        return -1;
    }

    if(fread(&net->layer[net->num_layers-1][0].bias_momentum, 1, sizeof(f16), f) != sizeof(f16))
    {
        fclose(f);
        return -1;
    }
